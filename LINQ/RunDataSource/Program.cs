...
class Program {
    static void Main(string[] args) {
        var thirdYear = from student in Sources.Students
        where student.Year == GradeLevel.ThirdYear
        select student;

        var TacomaTeachers = from teacher in Sources.Teachers
        where teacher.City == "Tacoma"
        select teacher;

        var teacherInDepartment = from teacher in Sources.Teachers
        join department in Sources.Departments on teacher.ID equals department.TeacherID
        where department.Name == "Chemistry"
        select teacher;
        // foreach (var student in thirdYear) {
        //     Console.WriteLine($"{student.FirstName} {student.LastName}");
        // }
        // foreach (var teacher in TacomaTeachers) {
        //     Console.WriteLine($"{teacher.First} {teacher.Last}");
        // }
        foreach (var teacher in teacherInDepartment) {
            Console.WriteLine($"{teacher.First} {teacher.Last}");
        }
    }
}